...
import { ILogger, DeviceError } from '../../module-0-core';
...
export class DeviceController {
    ...
    public async checkPosture(input: IDeviceInput): Promise<IDevicePosture> {
        ...
        try {
            ...
            this.logger.error(`Posture evaluation failed for ${input.deviceId}`, undefined, error as Error);
            throw new DeviceError(
                `Unable to evaluate posture for device ${input.deviceId}: ${(error as Error).message}`
            );
        }
    }
}
