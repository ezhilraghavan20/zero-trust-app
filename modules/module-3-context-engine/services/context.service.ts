...
export class ContextService {
    ...
    public async evaluateContext(input: IContextInput, identityKey: string = 'anonymous'): Promise<IContextRisk> {
        const locResult = this.locationSignal.process(input.location);
        const netResult = this.networkSignal.process(input.network);
        const timeResult = this.timeSignal.process(input.accessTime, identityKey, locResult.country);
        ...
        return {
            ...
            networkTrust: netResult.riskLevel === RiskLevel.NEGLIGIBLE ? 'TRUSTED' : 'UNTRUSTED',
